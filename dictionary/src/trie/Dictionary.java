...
public class Dictionary {
	...
	private void insertWord(String string, Node node) {//node =a,string=nal
		final Node nextChild;
		if (node.children.containsKey(string.charAt(0))) {//a has n;
			nextChild = node.children.get(string.charAt(0));//nextchildren=n
		} else {
			nextChild = new Node();
